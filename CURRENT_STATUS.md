...
2. **Project 页** — 自动检测 highlights（3K/4K/ACE/Clutch/Eco）
3. **Editor 页** — 视频播放 + 裁剪（In/Out 点 + 帧步进）+ 音频轨道导入 + 时间线编辑
4. **Export 页** — FFmpeg 导出（裁剪 → 合并 → 音频混合），支持取消和进度条
5. **Settings 页** — 设置持久化（electron-store）+ 受控组件 + 重置默认值 + Toast 通知
6. **Toast 通知系统** — 全局通知，支持 success/error/warning/info，自动关闭
7. **CS2 黄色主题** — 全应用 CS2 风格暗色主题（金色/深灰配色方案）

### 下一步应该做什么
见下方"待完成阶段"，**推荐优先级**：
1. **Phase 2.2~2.4 录制管线** — 需要 CS2 + OBS 环境，技术风险最高
2. **Phase 3.4 录制页 UI** — 依赖 Phase 2
...
- 设置项: language, cs2InstallPath, obsHost, obsPort, obsPassword, preRoll, postRoll
...
| 文件 | 说明 |
|------|------|
| `src/main/index.ts` | 主进程入口，IPC handlers，Python API 代理，`local-video://` 协议，导出+项目持久化+设置持久化 handler |
...
| `src/renderer/src/pages/WelcomePage.tsx` | 拖放 + 文件选择 + 视频导入 + 后端联动（CS2 主题） |
...
| `src/renderer/src/pages/SettingsPage.tsx` | 设置页（受控组件 + 持久化 + Toast 通知 + CS2 主题） |
...
| 文件 | 说明 |
|------|------|
| `src/shared/ipc.ts` | IPC 通道常量（含导出+项目持久化+设置持久化通道） |
...
| `src/preload/index.ts` | electronAPI 桥接（含导出+项目持久化+设置持久化方法） |
...
| P2 | Phase 2.2 | 2d | ⬜ 待开始 | CS2 控制 (CFG 生成 + 进程启动) — 需 CS2 环境 |
| P2 | Phase 2.3 | 1.5d | ⬜ 待开始 | OBS WebSocket 集成 — 需 OBS 环境 |
| P2 | Phase 2.4 | 2d | ⬜ 待开始 | 录制编排状态机 — 依赖 Phase 2.2 + 2.3 |
| P2 | Phase 3.4 | 1.5d | ⬜ 待开始 | 录制页 UI — 依赖 Phase 2 |
